...
def fsd50k_controls(fsd_50k: pd.DataFrame, mapping: dict) -> pd.DataFrame:
    """
    ...
    """
    control_classes = [k for k in mapping["Control"].keys()]
    fsd_50k_controls = fsd_50k[fsd_50k["labels"].isin(control_classes)]
    fsd_50k_controls.loc[:, "labels"] = fsd_50k_controls["labels"].apply(
        lambda x: mapping["Control"][str(x)]["foams_mapping"]
    )
    return fsd_50k_controls
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        default="../data/metadata/fsd50k_triggers_metadata.csv",
    ...
    print("Filtering FSD50K for control sounds...")
    fsd_50k_controls = fsd50k_controls(fsd_50k, data)
    print(f"Number of control samples found: {fsd_50k_controls.shape[0]}")

    print(f"Copying control samples to {args.target_dir}")
    num_copied = copy_samples(fsd_50k_controls, source_dir=args.source_dir, target_dir=args.target_dir)
    print(f"Copied {num_copied} samples.")

    print("Saving metadata...")
    save_metadata(fsd_50k_controls, target_path=args.metadata_path)
