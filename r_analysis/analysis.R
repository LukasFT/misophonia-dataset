...
# Ensure these columns are there:
# rating,subject_uuid,pair_uuid,is_trig,is_declared_trig,order,is_foams,is_anchor,is_reused,did_identify

required_columns <- c("rating", "subject_uuid", "pair_uuid", "is_trig", "is_declared_trig", "order", "is_foams", "is_anchor", "is_reused", "did_identify", "category", "version")
...
# Model per spec (REML; random slopes for is_trig by subject and item; uncorrelated)
form <- 
  rating ~ 
      # Fixed effects:
      is_trig + is_declared_trig + did_identify + order + is_foams + 
      # Random effects:
      # (1 + is_trig || subject_uuid) + (1 + is_trig || subject_uuid)
      (1 + is_trig || subject_uuid) + (1 | category)
